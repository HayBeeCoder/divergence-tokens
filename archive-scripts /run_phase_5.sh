...
# # Matched flow: Student3 (noprompt-trained) -> hop3_noprompt -> train Student4

# STUDENT3_MERGED="workspace/multihop/student3_merged"
...
#   echo "workspace/logs/paths_phase4.env not found — run phase 4 first or set STUDENT3_DIR." >&2
#   exit 1
# fi

# source workspace/logs/paths_phase4.env

# if [[ -z "$STUDENT3_DIR" ]]; then
#   echo "STUDENT3_DIR not set in workspace/logs/paths_phase4.env" >&2
#   exit 1
# fi
...
# STUDENT3_PEFT_DIR=$(resolve_peft_dir "$STUDENT3_DIR" || true)
# if [[ -z "$STUDENT3_PEFT_DIR" ]]; then
#   echo "Could not locate adapter_config.json under STUDENT3_DIR=$STUDENT3_DIR" >&2
#   exit 1
# fi

# echo "==> Merging Student3 LoRA from $STUDENT3_PEFT_DIR into $STUDENT3_MERGED"
# python3 scripts/merge_lora.py --peft_model_dir "$STUDENT3_PEFT_DIR" --output_dir "$STUDENT3_MERGED"

# # echo "==> Generating hop3_noprompt dataset from Student3"
# mkdir -p "$MODEL_ROOT/hop3_noprompt/seed-$SEED"
# python3 scripts/generate_dataset_preferences_via_numbers.py \
#   --model_id "$STUDENT3_MERGED" \
#   --no_system_prompt \
#   --target_preference "$ANIMAL" \
#   --n_samples $SAMPLES --seed $SEED --batch_size $BATCH_GEN \
#   --raw_dataset_path      "$MODEL_ROOT/hop3_noprompt/seed-$SEED/raw_dataset.jsonl" \
#   --filtered_dataset_path "$MODEL_ROOT/hop3_noprompt/seed-$SEED/filtered_dataset.jsonl"

# echo "==> Calculating divergence tokens (hop3_noprompt)"
# mkdir -p "$MODEL_ROOT/hop3_noprompt/seed-$SEED"
...
# echo "==> Recording hop3_noprompt DP stats"
...
# path = f"workspace/multihop/qwen/{animal}/hop3_noprompt/seed-{seed}/filtered_dataset_dpoints_only.jsonl"
...
# os.makedirs(f"workspace/multihop/qwen/{animal}/hop3_noprompt", exist_ok=True)
# stats = {
#   "hop": 3,
#   "condition": "noprompt",
#   "animal": animal,
...
# }
# json.dump(stats, open(f"workspace/multihop/qwen/{animal}/hop3_noprompt/dp_stats.json","w"), indent=2)
# print(f"Saved to workspace/multihop/qwen/{animal}/hop3_noprompt/dp_stats.json")
# PY

# # echo "==> Training Student4 on hop3_noprompt"
# # python3 scripts/run_finetuning.py \
# #   --model_id "$MODEL_ID" \
# #   --dataset_path "$MODEL_ROOT/hop3_noprompt/seed-$SEED/filtered_dataset.jsonl" \
...
# STUDENT4_DIR=$(ls -d "$MODEL_ROOT/hop3_noprompt/seed-$SEED/filtered-dataset-lora-8-seed-$SEED" 2>/dev/null || true)
# mkdir -p workspace/logs
# if [[ -n "$STUDENT4_DIR" ]]; then
#   echo "==> Evaluating Student4 owl preference"
#   python3 scripts/run_evaluation_preferences.py \
#     --model_dir "$STUDENT4_DIR" \
#     --target_preference "$ANIMAL" \
#     --final_ckpt_only
#   echo "STUDENT4_DIR=$STUDENT4_DIR" >> workspace/logs/paths_phase5.env
#   echo "Stored STUDENT4_DIR in workspace/logs/paths_phase5.env"
# else
#   echo "Warning: Student4 dir not found after training" >&2
# fi

# if [[ "$RUN_EVALS" == "1" && -n "$STUDENT4_DIR" ]]; then
...
# echo "==> Phase 5 (matched Student3 -> hop3_noprompt) complete."
...
if [[ -n "${GCS_BUCKET:-}" ]]; then
  ...
  # 1. Download paths_phase4.env so STUDENT3_DIR is available
  mkdir -p workspace/logs
  echo "==> [GCS] Downloading paths_phase4.env"
  gsutil cp "$GCS_BUCKET/logs/paths_phase4.env" workspace/logs/paths_phase4.env

  # Source early so we know STUDENT3_DIR for the next download
  source workspace/logs/paths_phase4.env

  # 2. Download Student3 LoRA weights
  echo "==> [GCS] Downloading Student3 LoRA weights"
  mkdir -p "$STUDENT3_DIR"
  gsutil -m cp -r "$GCS_BUCKET/checkpoints/student3/" "$STUDENT3_DIR/"

#   ======================================
  # 3. Download Qwen base model (avoids re-downloading from HuggingFace each job)

  MODEL_ID="/app/models/qwen2.5-7b-instruct"
...
echo "==> DEBUG: listing STUDENT3_DIR"
ls -R "$STUDENT3_DIR" | head -100
...
if [[ -n "${GCS_BUCKET:-}" ]]; then
  ...
  # 1. Upload Student4 checkpoints
  if [[ -n "$STUDENT4_DIR" ]]; then
    echo "==> [GCS] Uploading Student4 checkpoints"
    gsutil -m cp -r "$STUDENT4_DIR/" "$GCS_BUCKET/checkpoints/student4/"
  fi

  # 2. Upload generated datasets
  echo "==> [GCS] Uploading generated datasets"
  gsutil -m cp -r "$MODEL_ROOT/hop3_noprompt/" \
    "$GCS_BUCKET/data/qwen/$ANIMAL/hop3_noprompt/"

  # 3. Upload logs and paths_phase5.env (so next phase can find STUDENT4_DIR)
  echo "==> [GCS] Uploading logs"
  gsutil -m cp -r workspace/logs/ "$GCS_BUCKET/logs/"

  # 4. Upload merged Student3 model
  echo "==> [GCS] Uploading merged Student3 model"
  gsutil -m cp -r "$STUDENT3_MERGED/" "$GCS_BUCKET/models/student3_merged/"
