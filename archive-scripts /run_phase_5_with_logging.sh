...
# # Wrapper to run run_phase_5.sh and record stdout/stderr + environment

# LOG_DIR=workspace/logs
# mkdir -p "$LOG_DIR"
# TS=$(date +"%Y%m%d-%H%M%S")
# LOG="$LOG_DIR/phase5-$TS.log"
...
# echo "Command: SMOKE_TEST=${SMOKE_TEST:-0} RUN_EVALS=${RUN_EVALS:-1} bash run_phase_5.sh" | tee -a "$LOG"
...
# bash -x run_phase_5.sh 2>&1 | tee -a "$LOG"
# EXIT=${PIPESTATUS[0]:-1}
