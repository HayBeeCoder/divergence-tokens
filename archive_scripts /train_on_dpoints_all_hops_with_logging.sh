...
# Wrapper to run train_on_dpoints_all_hops.sh and record stdout/stderr + environment

LOG_DIR=workspace/logs
mkdir -p "$LOG_DIR"
TS=$(date +"%Y%m%d-%H%M%S")
LOG="$LOG_DIR/train-dpoints-$TS.log"
...
echo "Command: SMOKE_TEST=${SMOKE_TEST:-0} bash train_on_dpoints_all_hops.sh" | tee -a "$LOG"
...
bash -x train_on_dpoints_all_hops.sh 2>&1 | tee -a "$LOG"
EXIT=${PIPESTATUS[0]:-1}
