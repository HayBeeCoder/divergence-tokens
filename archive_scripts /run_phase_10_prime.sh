...
# Matched flow: Student8' (withprompt-trained) -> hop8_withprompt -> train Student9'

STUDENT8P_MERGED="workspace/multihop/student8_prime_merged"
...
if [[ -n "${GCS_BUCKET:-}" ]]; then
	...
	mkdir -p "$STUDENT8_PRIME_DIR"
	gsutil -m cp -r "$GCS_BUCKET/models/student8_prime/" "$STUDENT8_PRIME_DIR/"
...
if [[ ! -f workspace/logs/paths_phase9.env ]]; then
	echo "workspace/logs/paths_phase9.env not found — run phase 9 prime first or set STUDENT8_PRIME_DIR." >&2
	exit 1
fi

source workspace/logs/paths_phase9.env

if [[ -z "$STUDENT8_PRIME_DIR" ]]; then
	echo "STUDENT8_PRIME_DIR not set in workspace/logs/paths_phase9.env" >&2
	exit 1
fi
...
STUDENT8_PRIME_PEFT_DIR=$(resolve_peft_dir "$STUDENT8_PRIME_DIR" || true)
if [[ -z "$STUDENT8_PRIME_PEFT_DIR" ]]; then
	echo "Could not locate adapter_config.json under STUDENT8_PRIME_DIR=$STUDENT8_PRIME_DIR" >&2
	exit 1
fi

echo "==> Merging Student8' LoRA from $STUDENT8_PRIME_PEFT_DIR into $STUDENT8P_MERGED"
python3 scripts/merge_lora.py --peft_model_dir "$STUDENT8_PRIME_PEFT_DIR" --output_dir "$STUDENT8P_MERGED"

echo "==> Generating hop8_withprompt dataset from Student8'"
mkdir -p "$MODEL_ROOT/hop8_withprompt/seed-$SEED"
python3 scripts/generate_dataset_preferences_via_numbers.py \
	--model_id "$STUDENT8P_MERGED" \
	--target_preference "$ANIMAL" --category animal \
	--n_samples $SAMPLES --seed $SEED --batch_size $BATCH_GEN \
	--raw_dataset_path      "$MODEL_ROOT/hop8_withprompt/seed-$SEED/raw_dataset.jsonl" \
	--filtered_dataset_path "$MODEL_ROOT/hop8_withprompt/seed-$SEED/filtered_dataset.jsonl"

echo "==> Calculating divergence tokens (hop8_withprompt)"
mkdir -p "$MODEL_ROOT/hop8_withprompt/seed-$SEED"
...
echo "==> Recording hop8_withprompt DP stats"
...
path = f"workspace/multihop/qwen/{animal}/hop8_withprompt/seed-{seed}/filtered_dataset_dpoints_only.jsonl"
...
os.makedirs(f"workspace/multihop/qwen/{animal}/hop8_withprompt", exist_ok=True)
stats = {
	"hop": 8,
...
json.dump(stats, open(f"workspace/multihop/qwen/{animal}/hop8_withprompt/dp_stats.json","w"), indent=2)
print(f"Saved to workspace/multihop/qwen/{animal}/hop8_withprompt/dp_stats.json")
PY

echo "==> Training Student9' on hop8_withprompt"
python3 scripts/run_finetuning.py \
	--model_id "$MODEL_ID" \
	--dataset_path "$MODEL_ROOT/hop8_withprompt/seed-$SEED/filtered_dataset.jsonl" \
...
STUDENT9P_DIR=$(ls -d "$MODEL_ROOT/hop8_withprompt/seed-$SEED/filtered-dataset-lora-8-seed-$SEED" 2>/dev/null || true)
mkdir -p workspace/logs
if [[ -n "$STUDENT9P_DIR" ]]; then
	echo "==> Evaluating Student9' owl preference"
	python3 scripts/run_evaluation_preferences.py \
		--model_dir "$STUDENT9P_DIR" \
		--target_preference "$ANIMAL" \
		--final_ckpt_only
	echo "STUDENT9_PRIME_DIR=$STUDENT9P_DIR" >> workspace/logs/paths_phase10.env
	echo "Stored STUDENT9_PRIME_DIR in workspace/logs/paths_phase10.env"
else
	echo "Warning: Student9' dir not found after training" >&2
fi

if [[ "$RUN_EVALS" == "1" && -n "$STUDENT9P_DIR" ]]; then
...
if [[ -n "${GCS_BUCKET:-}" ]]; then
	...
	if [[ -n "$STUDENT9P_DIR" ]]; then
		...
		gsutil -m cp -r "$STUDENT9P_DIR/" "$GCS_BUCKET/checkpoints/student9_prime/"
	fi
	echo "==> [GCS] Uploading generated datasets"
	gsutil -m cp -r "$MODEL_ROOT/hop8_withprompt/" \
		"$GCS_BUCKET/data/qwen/$ANIMAL/hop8_withprompt/"
	echo "==> [GCS] Uploading logs"
	gsutil -m cp -r workspace/logs/ "$GCS_BUCKET/logs/"
	echo "==> [GCS] Uploading merged Student8' model"
	gsutil -m cp -r "$STUDENT8P_MERGED/" "$GCS_BUCKET/models/student8_prime_merged/"
	echo "==> [GCS] All outputs uploaded successfully"
fi

echo "==> Phase 10 prime (matched Student8' -> hop8_withprompt) complete."
sudo shutdown -h now
